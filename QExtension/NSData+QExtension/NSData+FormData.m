 ...
 *
 *  @param request      网络请求
 *  @param fileBoundary 文件分隔符，nil 默认设置为 @"QianChiaUploadBoundary"
 */
+ (void)q_setHttpHeaderFieldWithRequest:(NSMutableURLRequest *)request
                           fileBoundary:(nullable NSString *)fileBoundary {
    
    [request setValue:[NSString stringWithFormat:@"multipart/form-data; charset=utf-8; boundary=%@",
                       fileBoundary ? : QIAN_UPLOAD_BOUNDARY]
...
- (void)q_appendPartWithFileData:(NSData *)fileData
    ...
    [self appendData:[[NSString stringWithFormat:@"Content-Type: %@\r\n",
                       (mimeType && mimeType != (id)[NSNull null]) ? mimeType : @"application/octet-stream"]
                      dataUsingEncoding:NSUTF8StringEncoding]];
    [self appendData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]];
